...
def test_hmm_on_example_sentence():
    ...
    score, pred_tags = viterbi.build_trellis(all_tags,
                                             tag_to_ix,
                                             [emission_probs[word_to_ix[w]] for w in
                                              ['they', 'can', 'can', 'fish', '.']],
...
def test_hmm_dev_accuracy():
    confusion = scorer.get_confusion(DEV_FILE, prefix + 'hmm-dev-en.preds')
    acc = scorer.accuracy(confusion)
    ok_(acc > .840, "Accuracy lower than 0.840")


# 4.3b
def test_hmm_test_accuracy():
    confusion = scorer.get_confusion(TEST_FILE, prefix + 'hmm-te-en.preds')
    acc = scorer.accuracy(confusion)
    ok_(acc > .840, "Accuracy lower than 0.840")
...
def test_nr_hmm_dev_accuracy():
    confusion = scorer.get_confusion(NR_DEV_FILE, prefix + 'hmm-dev-nr.preds')
    acc = scorer.accuracy(confusion)
    ok_(acc > .861, "Accuracy lower than 0.861")
...
def test_nr_hmm_test_accuracy():
    ...
    acc = scorer.accuracy(confusion)
    ok_(acc > .853, "Accuracy lower than 0.853")
