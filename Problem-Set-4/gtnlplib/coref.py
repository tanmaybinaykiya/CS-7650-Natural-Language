...
def get_markables_for_entity(markables, entity):
    ...
    :rtype: list
    """
    return [" ".join(markable.string) for markable in markables if markable.entity == entity]
...
def get_tp(pred_ant, markables):
    ...
    """
    # assert len(pred_ant) == len(markables)
    arr = []
...
def read_data(page_name, basedir=os.path.join('data', 'wsj', 'tr'), max_markables=300, tagger=None):
    """
