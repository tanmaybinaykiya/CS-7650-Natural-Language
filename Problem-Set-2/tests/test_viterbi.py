...
def test_viterbi_step_init():
    ...
    viterbi_vars, bptrs = viterbi.viterbi_step(all_tags, tag_to_ix, emission_probs[1], tag_transition_probs, prev_scores)

    eq_(viterbi_vars[1].data.numpy(), -10)
    eq_(viterbi_vars[2].data.numpy(), -6)
    eq_(bptrs[1], 1)
    eq_(bptrs[2], 1)
    eq_(bptrs[3], 0)
...
def test_trellis_score():
    ...
    print("best_path: ", list(best_path))
